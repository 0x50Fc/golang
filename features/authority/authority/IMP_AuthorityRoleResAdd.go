...
func (S *Service) AuthorityRoleResAdd(app micro.IContext, task *AuthorityRoleResAddTask) (*Authority, error) {
	...
	err = db.Transaction(conn, func(conn db.Database) error {
		...
		_, err := db.Get(conn, &v, prefix, " WHERE uid=0 AND roleid=? AND resid=?", task.RoleId, task.ResId)
