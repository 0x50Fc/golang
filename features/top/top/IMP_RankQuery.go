...
func (S *Service) RankQuery(app micro.IContext, task *RankQueryTask) (*QueryData, error) {
	...
	if p == 1 && task.Tids == nil && task.TopId == nil && task.Q == nil {

		{
			...
			if err == nil {
				...
				text, err := redis.Get(fmt.Sprintf("%s%s_rank", prefix, task.Name)).Result()
	...
	sql.WriteString(fmt.Sprintf(" ORDER BY `rank` ASC,sid DESC,id DESC LIMIT %d,%d", (p-1)*n, n))
	...
	if data.Page != nil {
		if task.TopId != nil {
			...
			data.Page.TopId = int64(data.Items[0].Rank)
		}
	}
	...
	if p == 1 && task.Tids == nil && task.TopId == nil && task.Q == nil {
		...
		if err == nil {
			...
			redis.Set(fmt.Sprintf("%s%s_rank", prefix, task.Name), string(b), time.Second*time.Duration(maxSecond)).Result()
